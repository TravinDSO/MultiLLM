...
from llms.agents.confluence_agent import OllamaConfluenceAgent
...
class AOHybridOrchestrator(AzureMulti):
    def __init__(self, api_key,model='gpt-4o',endpoint='',version='',info_link='',type='assistant', wait_limit=300,
        ...
        #Agents
        self.confluence_agent = OllamaConfluenceAgent(model='llama3.1:latest',confluence_url=confluence_url,confluence_token=confluence_token)
    ...
    def handle_tool(self, user, tool):
        ...
        if tool_name == "generate_image":
            ...
            self.extra_messages[user].append(f'<HR><i>Asking the Agent Writer (Ollama): {args["prompt"]}</i>')
            self.llama3_agent.agent_instructions = "You are a professional writer. Use the information and instructions provided to write a response."
            results = self.llama3_agent.generate(user, args['prompt'])
        elif tool_name == "agent_researcher":
            self.extra_messages[user].append(f'<HR><i>Asking the Agent Researcher (Ollama): {args["prompt"]}</i>')
            self.llama3_agent.agent_instructions = "You are a professional researcher and analyist. Use the information and instructions provided to research and provide feedback."
            results = self.llama3_agent.generate(user, args['prompt'])
            ...
            self.extra_messages[user].append(f'<HR><i>Asking the Agent Confluence (Ollama): {args["prompt"]}</i>')
    ...
    def clear_conversation(self, user):
        ...
        self.llama3_agent.clear_conversation(user)
